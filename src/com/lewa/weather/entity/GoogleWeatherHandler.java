	...
	public void startElement(String namespaceURI, String localName,
		...
		if (localName.equals("forecast_information")) {
			...
			if (localName.equals("city")) {
				this.myWeatherSet.setCity(dataAttribute);
			} else if (localName.equals("postal_code")) {
				this.myWeatherSet.setPostal_code(dataAttribute);
			} else if (localName.equals("latitude_e6")) {
				/* One could use this to convert city-name to Lat/Long. */
				this.myWeatherSet.setLatitude(dataAttribute);
			} else if (localName.equals("longitude_e6")) {
				/* One could use this to convert city-name to Lat/Long. */
				this.myWeatherSet.setLongitude(dataAttribute);
			} else if (localName.equals("forecast_date")) {
				this.myWeatherSet.setForecast_date(dataAttribute);
			...
			// "<forecast_conditions>"
				...
				if (this.in_current_conditions) {
					...
					this.myWeatherSet.getWeatherCurrentCondition().setIconName(dataAttribute);
				} else if (this.in_forecast_conditions) {
					this.myWeatherSet.getLastWeatherForecastCondition().setIconURL(dataAttribute);
					this.myWeatherSet.getLastWeatherForecastCondition().setIconName(dataAttribute);
				}
			...
			else if (localName.equals("tempurature")) {
				if (this.in_current_conditions) {
					this.myWeatherSet.getWeatherCurrentCondition()
							.setTemperature(dataAttribute);
				} else if (this.in_forecast_conditions) {
					this.myWeatherSet.getLastWeatherForecastCondition()
							.setTemperature(dataAttribute);
				}
